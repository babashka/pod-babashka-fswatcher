   ...
   ret)))`,
	...
	case "invoke":
		...
		case "pod.babashka.fswatcher/-create-watcher":
			...
			if watcher, err := createWatcher(message, path, opts); err != nil {
				babashka.WriteErrorResponse(message, err)
			} else {
				babashka.WriteInvokeResponse(message, watcher)
			}
		...
		case "pod.babashka.fswatcher/-start-watcher":
			...
			babashka.WriteInvokeResponse(message, startWatcher(message, args[0]))
